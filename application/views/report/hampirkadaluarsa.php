		...
		<div class="row">
			<div class="col-md-12">
				<div class="card card-primary">
					...
					<div class="card-body">
						<a href="<?= base_url() . 'export/exportStokHampirKadaluarsa' ?>" class="btn btn-info btn-rounded">Export Excel</a>
						<a href="<?= base_url() . 'cetak/laporanhampirkadaluarsa' ?>" target="_blank" class="btn btn-secondary btn-neutral">Print</a>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function() {
		...
		const table = $('#tbl-barang').DataTable({
			...
			ajax: {
				url: "getHampirKadaluarsa", // URL file untuk proses select datanya
				type: "POST"
			},
			columns: [{
					data: "kode_barang",
